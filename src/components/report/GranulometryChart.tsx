...
const xMin=0.01,xMax=100;
function xPos(x:number){const a=Math.log10(xMin),b=Math.log10(xMax);return M.left+((Math.log10(x)-a)/(b-a))*(W-M.left-M.right)}
function yPos(y:number){return M.top+((100-y)/100)*(H-M.top-M.bottom)}
function polyline(points:Array<{x:number;y:number}>){return points.map(p=>`${xPos(p.x).toFixed(2)},${yPos(p.y).toFixed(2)}`).join(" ")}
const xGrid:number[]=[];for(const decade of [0.01,0.1,1,10])for(let m=1;m<=9;m++)xGrid.push(decade*m);xGrid.push(100);
const bottomLabels=[0.01,0.1,1,10,100];
export function GranulometryChart({band,calculation}:{band:TrackBand;calculation:GranulometryCalculation}){
 ...
 const obtained=calculation.rows.filter(r=>r.passingPct!==null).map(r=>({x:r.mm,y:r.passingPct as number})).sort((a,b)=>a.x-b.x);
 const lower=defs.map(r=>({x:r.mm,y:100-r.maxRetainedAccum})).sort((a,b)=>a.x-b.x);
 const upper=defs.map(r=>({x:r.mm,y:100-r.minRetainedAccum})).sort((a,b)=>a.x-b.x);
 ...
 return <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" role="img" aria-label="Curva granulométrica">
  ...
  {xGrid.map(v=>{const major=[0.01,0.1,1,10,100].includes(Number(v.toPrecision(3)));return <line key={`x${v}`} x1={xPos(v)} x2={xPos(v)} y1={M.top} y2={H-M.bottom} stroke={major?"#555":"#b8b8b8"} strokeWidth={major?.8:.4}/>})}
  {MASTER_SIEVES.map(s=><g key={s.mm}><line x1={xPos(s.mm)} x2={xPos(s.mm)} y1={M.top} y2={H-M.bottom} stroke="#333" strokeDasharray="4 3" strokeWidth="1"/><text x={xPos(s.mm)} y={M.top-11} fontSize="12" textAnchor="middle" fontWeight="700">{s.label}</text></g>)}
