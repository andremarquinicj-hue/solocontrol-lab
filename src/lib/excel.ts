...
function addSummarySheet(workbook: ExcelJS.Workbook, data: LabReportData, calc: LabReportCalculation): string {
  ...
  calc.summaries.forEach((s, i) => {
    ...
    const st = statusFill(s.status); ws.getCell(r, 6).fill = { type: "pattern", pattern: "solid", ...st.fill }; ws.getCell(r, 6).font = { bold: true, color: { argb: st.font } };
  });
  ...
  ws.mergeCells(total, 2, total, 6); ws.getCell(total, 2).value = calc.overallStatus; const st = statusFill(calc.overallStatus); ws.getCell(total, 2).fill = { type: "pattern", pattern: "solid", ...st.fill }; ws.getCell(total, 2).font = { bold: true, color: { argb: st.font }, size: 11 }; ws.getCell(total, 2).alignment = { horizontal: "center" };
...
function addGranulometrySheet(workbook: ExcelJS.Workbook, data: LabReportData, calc: GranulometryCalculation, chartBase64?: string): string {
  ...
  calc.rows.forEach((r, i) => {
    ...
    const st=statusFill(r.conformityStatus); ws.getCell(rr,13).fill={type:"pattern",pattern:"solid",...st.fill}; ws.getCell(rr,13).font={bold:true,color:{argb:st.font},size:9};
  });
...
function officialSectionStatus(ws:ExcelJS.Worksheet,row:number,label:string,ref:string,result:string,criterion:string,status:string){
 ws.getCell(row,1).value=label;ws.mergeCells(row,2,row,5);ws.getCell(row,2).value=ref;ws.getCell(row,6).value=result;ws.mergeCells(row,6,row,7);ws.getCell(row,8).value=criterion;ws.mergeCells(row,8,row,9);ws.getCell(row,10).value=status;styleDataArea(ws,row,row,1,10);const st=statusFill(status);ws.getCell(row,10).fill={type:"pattern",pattern:"solid",...st.fill};ws.getCell(row,10).font={bold:true,color:{argb:st.font},size:9};
}
...
function addOfficialReportCells(workbook:ExcelJS.Workbook,ws:ExcelJS.Worksheet,data:LabReportData,calc:LabReportCalculation,refs:FormulaRefs,options:ExcelBuildOptions){
 ...
 pages.push((row,page,total)=>{let r=reportHeader(workbook,ws,data,row,"Matriz de conformidade e conclusão",page,total,options.logoBase64);sectionTitle(ws,r,"MATRIZ DE CONFORMIDADE • RESULTADOS CONSOLIDADOS");r+=2;ws.getRow(r).values=["Ensaio","Resultado","Unidade","Critério","Método / referência","","","","","Situação"];styleTableHeader(ws.getRow(r),1,10);calc.summaries.forEach(s=>{r++;ws.getCell(r,1).value=s.label;ws.getCell(r,2).value=s.result;ws.getCell(r,3).value=s.unit;ws.getCell(r,4).value=s.criterion;ws.mergeCells(r,5,r,9);ws.getCell(r,5).value=s.reference;ws.getCell(r,10).value=s.status;styleDataArea(ws,r,r,1,10);const st=statusFill(s.status);ws.getCell(r,10).fill={type:"pattern",pattern:"solid",...st.fill};ws.getCell(r,10).font={bold:true,color:{argb:st.font}};});r+=2;ws.mergeCells(r,1,r,4);ws.getCell(r,1).value="CONFORMIDADE GERAL";ws.getCell(r,1).fill={type:"pattern",pattern:"solid",fgColor:{argb:BLUE}};ws.getCell(r,1).font={bold:true,color:{argb:WHITE}};ws.mergeCells(r,5,r,10);ws.getCell(r,5).value=calc.overallStatus;const st=statusFill(calc.overallStatus);ws.getCell(r,5).fill={type:"pattern",pattern:"solid",...st.fill};ws.getCell(r,5).font={bold:true,color:{argb:st.font},size:12};ws.getCell(r,5).alignment={horizontal:"center"};for(let c=1;c<=10;c++)applyBorder([ws.getCell(r,c)]);return reportFooter(ws,r+3,data);});
